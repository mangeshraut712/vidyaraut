    ...
    try {
      ...
      if (healthResponse.ok) {
        try {
          ...
          // Primary indicator: check ai_status from health endpoint
          if (healthData.ai_status === 'configured') {
            aiStatus = 'online';
          } else if (healthData.ai_status === 'fallback_only') {
            aiStatus = 'degraded';
          }
        } catch (parseError) {
          console.warn('Failed to parse health response:', parseError);
          // If we can't parse but got 200, assume AI is online
          aiStatus = 'online';
        }
      }

      // Secondary verification: test AI with a simple message
      // Only run if health check indicated AI is online
      if (aiStatus === 'online') {
        try {
          ...
          // If test fails with server error or auth error, mark as degraded
          if (testResponse.status >= 500 || testResponse.status === 401) {
            aiStatus = 'degraded';
          }
          // For any other status (including 429, 400, etc), keep as online
          // since health check already confirmed AI is configured
        } catch (testError) {
          // Network error or timeout - keep as online since health check passed
          console.warn('AI test request failed:', testError.message);
        }
      }
